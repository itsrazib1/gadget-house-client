...
const Header = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`https://gadget-house-server.vercel.app/main?q=${searchQuery}`);
      ...
      <div className='relative flex items-center justify-between'>
        ...
        <div className='flex items-center space-x-4'>
          ...
          {searchResults.length > 0 && (
            <div className='absolute z-10 top-full -left-8 mt-2 bg-white border rounded shadow-sm'>
              <ul className='space-y-2 gap-10   grid grid-cols-2 sm:grid-cols-6'>
                {searchResults
                  .filter((result) =>
                    ...
                    <ul className=''>
                      <li key={result.id}>
                        <Link to={`/`}>
                          ...
                          <div className="card  text-center p-4 h-[340px] w-52 sm:w-60 bg-base-200 shadow-xl image-full">
                            <figure><img className='h-36' src={result.picture} alt="Shoes" /></figure>

                            <div className="card-body">
                              <h2 className="card-title">{result.name}</h2>
                              <p>${result.price}</p>
                              <div className=" justify-end">
                                <button className="btn btn-primary text-xs"><Link to="/shop">Buy Now</Link></button>
                              </div>
                            </div>
                          </div>
                        </Link>
